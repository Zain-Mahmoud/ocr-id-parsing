...
### Questions
We have access to an A100 GPU but it is currently being used by other services and might not be the most efficient, however GPU based services are still an option. Which plan is the best? Why? What would you change? What are the bottlenecks and what will actually impact performance. How many models should we use? Where should we settle for pre-trained vs fine-tune ourselves? We also have access to Kaggle and Google Colab free GPUs for training purposes. you may ask for more context if needed